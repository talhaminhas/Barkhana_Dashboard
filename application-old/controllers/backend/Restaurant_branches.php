...
/**
 * Restaurant_branches Controller
 */
class Restaurant_branches extends BE_Controller {
	...
	function __construct() {
		...
		parent::__construct( MODULE_CONTROL, 'RESTAURANT_BRANCHES' );
	...
	function index() {
		...
		// get rows count
		$this->data['rows_count'] = $this->Restaurant_branch->count_all_by( $conds );

		// get branches
		$this->data['branches'] = $this->Restaurant_branch->get_all_by( $conds , $this->pag['per_page'], $this->uri->segment( 4 ) );
	...
	function save( $id = false ) {
		...
		//save category
		if ( ! $this->Restaurant_branch->save( $data, $id )) {
	...
	*/
	function edit( $id ) 
	{
		...
		// load user
		$this->data['branch'] = $this->Restaurant_branch->get_one( $id );
	...
	function delete( $restaurant_id ) {
	    ...
	    if ( ! $this->ps_delete->delete_restaurant_branch( $restaurant_id, $enable_trigger )) {
	...
	{		
		 $conds['name'] = $name;
		if ($id) {
			if ( strtolower( $this->Restaurant_branch->get_one( $id )->name ) == rtrim(strtolower( $name ))) {
				
			// if the name is existing name for that user id,
				return true;
			} else if ( $this->Restaurant_branch->exists( ($conds ))) {
			...
			if ( $this->Restaurant_branch->exists( ($conds ))) {
	...
	function ajx_publish( $restaurant_id = 0 )
	{
		...
		// save data
		if ( $this->Restaurant_branch->save( $restaurant_data, $restaurant_id )) {
			echo true;
		} else {
			echo false;
		}
