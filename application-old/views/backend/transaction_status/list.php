<div class="table-responsive animated fadeInRight">
	...
	<?php if ( !empty( $transactionstatus ) && count( $transactionstatus->result()) > 0 ): ?>

		<?php foreach($transactionstatus->result() as $trans_status): ?>
			
			<tr>
				<td><?php echo ++$count;?></td>
				<td ><?php echo $trans_status->title;?></td>

				<?php if ( $this->ps_auth->has_access( EDIT )): ?>
			
					<td>
						<a href='<?php echo $module_site_url .'/edit/'. $trans_status->id; ?>'>
				...
				<?php if ( $this->ps_auth->has_access( DEL )): ?>
					...
					<td>
						<a herf='#' class='btn-delete' data-toggle="modal" data-target="#myModal" id="<?php echo "$trans_status->id";?>">
