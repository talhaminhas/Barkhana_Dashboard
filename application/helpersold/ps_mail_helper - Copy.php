	...
	function send_transaction_order_emails( $trans_header_id, $to_who = "", $subject = "" )
	{
		...
		for($i=0;$i<count($trans_details_obj);$i++) 
		{
				if($trans_details_obj[$i]->product_addon_id != "") {
						...
						for($k = 0; $k < count($att_name_info); $k++) {
							...
							if($att_name_info[$k] != "") {
								...
								<div style='font-weight: bold;'>
									<span >". $att_name_info[$k] .":</span> £". $addon_price ." 
								</div>
							";
					...
					$att_info_str = "<li style='
						...
						margin: 10px 0;
						list-style: none;
						'><div style='font-size: 18px; font-weight: bold; color: #ff5733; margin-bottom: 10px'> Extras</div>"
						.$addon_content.
						"</li>";
				...
				$total_amount = number_format((float)$trans_details_obj[$i]->original_price * (int)$trans_details_obj[$i]->qty, 2, '.', '');
				...
				list-style: none;
				'>
				<div style='font-size: 18px; font-weight: bold; color: #ff5733; margin-bottom: 10px'>{$trans_details_obj[$i]->product_name}</div>
				<ul style='padding: 0;'>{$att_info_str}</ul>
				<div style='font-weight: bold;'>
					<span >Quantity:</span> {$trans_details_obj[$i]->qty}
				</div>
				<div style='font-weight: bold;'>
					<span >Unit Price:</span> £". $original_price ."
				</div>
				<div style='font-weight: bold;'>
					<span>Total: </span>£" 
		...
		if ($to_who == "shop") {
			...
			<div style='
				...
				margin: 10px 0;
			'>
				<p style='font-size: 18px; font-weight: bold; color: #ff5733;'><strong>{$prd_detail_info}:</strong></p>
				<div style='
					background-color: #fabcb9;
					border: 0px solid #ddd;
					border-radius: 10px;
				...
				'>
					{$order_items}
					<p><strong>{$sub_total}: £{$sub_total_amt}</br>
					{$coupon_dis_amount}: - £{$coupon_discount_amount}</br>
					{$shipping_cost}: + £{$shipping_amount}</strong></p>
				</div>
				<p style='font-size: 18px; font-weight: bold; color: #ff5733;'>{$total_bal_amt}: £{$total_balance_amount}</p>
			</div>
		...
		EOL;
			...
			'>
				...
				'>
					{$order_items}
					<p><strong>{$sub_total}: £{$sub_total_amt}</strong></p>
					<p><strong>{$coupon_dis_amount}: - £{$coupon_discount_amount}</strong></p>
					<p><strong>{$shipping_cost}: + £{$shipping_amount}</strong></p>
				</div>
				<p style='font-size: 18px; font-weight: bold; color: #ff5733;'>{$total_bal_amt}: £{$total_balance_amount}</p>
			</div>
			<p>{$best_regards},<br>{$sender_name}</p>
