<div class="table-responsive animated fadeInRight">
	<table class="table m-0 table-striped">
		<tr>
			...
			<th><?php echo get_msg('popular_cat_name'); ?></th>
			<th><?php echo get_msg('popular_cat_img'); ?></th>
	...
	<?php if ( !empty( $popularcategories ) && count( $popularcategories->result()) > 0 ): ?>
		...
		<?php foreach($popularcategories->result() as $popularcategory): ?>
			...
			<tr>
				...
				$default_photo = get_default_photo( $popularcategory->id, 'category' );

				if($default_photo->img_path != "") {
				 ?>		
				

				<td><img class="img-rounded img-fluid" width="128" height="128" src="<?php echo img_url( ''. $default_photo->img_path ); ?>"/></td>
