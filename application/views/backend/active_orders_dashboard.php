<div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true" class="table-responsive animated fadeInRight" style="padding-top: 10px;">
    ...
    <li class="nav-item">
        <a class="nav-link active  font-weight-bold" data-toggle="tab" href="#new_orders_tab" role="tab">New Orders</a>
    </li>
		<li class="nav-item">
			<a class="nav-link font-weight-bold" data-toggle="tab" href="#ongoing_orders_tab" role="tab">Ongoing Orders</a>
		</li>
	</ul>
	<div class="tab-content">
		<div class="tab-pane active" id="new_orders_tab" role="tabpanel">
			<!-- New Orders content goes here -->
			<!-- New Orders  -->
		...
		<div class="tab-pane" id="ongoing_orders_tab" role="tabpanel">
			<!-- Ongoing Orders content goes here -->
			<!-- Ongoing Orders  -->
...
<script>
	  var isRefreshing = true; // Flag to track if auto-refresh is active

		// Function to start or stop the auto-refresh
		function toggleRefresh(isActive) {
			if (isActive) {
				// Start auto-refresh
				isRefreshing = true;
				refresh();
		...
		function handleTabSwitch(target) {
			if (target === "#new_orders_tab") {
				// Code to run when New Orders tab is shown
				toggleRefresh(true);
			} else if (target === "#ongoing_orders_tab") {
				// Code to run when Ongoing Orders tab is shown
				toggleRefresh(false); // Stop auto-refresh
			}
		}
