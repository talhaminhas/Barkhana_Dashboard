	...
	function login_post()
	{
        ...
        if ( !$this->User->exists( array( 'user_email' => $this->post( 'user_email' ), 'user_password' => $this->post( 'user_password' )))) {
	        ...
	        if ( $code != "" && $code != " "){
        		...
        		$token = $this->jwtlib->generateToken($user_info->user_email);//JWT::encode($payload, $key, 'HS256');

				$data=$this->User->get_one_by(array("user_email" => $this->post('user_email')));
	...
	*/
	function reset_post()
	{
		...
		$msg = "<!DOCTYPE html>
				...
				<a class='reset-link' href='" . site_url( $this->config->item( 'reset_url') . '/' . $code ) . "'>" . get_msg( 'reset_link_label' ) . "</a>
				</p>
		...
		$msg1 = "<p>".$hi.",". $user_info->user_name ."</p>".
					"<p>".get_msg( 'pwd_reset_link' )."<br/>".
					"<a href='". site_url( $this->config->item( 'reset_url') .'/'. $code ) ."'>".get_msg( 'reset_link_label' )."</a></p><br/>".
					"<p>".get_msg( 'best_regards_label' ).",<br/>". $sender_name ."</p>";
